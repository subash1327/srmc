...
const appActions = {
  ...
  const tt = new AccessToken(LIVEKIT_API_KEY, LIVEKIT_API_SECRET, {
    identity,
  });
  tt.addGrant({
    room: room,
...
async function initiate() {
  ...
  // (<HTMLInputElement>$('token')).value = tt;
  sleep(1000)
  appActions.connectUser()
}
